...
exports.run = async (client, message, args) => {
  ...
  if(message.channel.id !== "830411405844283422") return;
...
if (!kullanıcı) return message.channel.send(new MessageEmbed().setDescription(`Bir kullanıcıyı sese çekmek istiyorsan o kullanıcıyı belirtmen @RK/İD gibi.`).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('RANDOM').setTimestamp()).then(x => x.delete({timeout: 5000}));
