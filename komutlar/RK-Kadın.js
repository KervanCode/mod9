...
exports.run = async (client, message, args) => {
 ...
 if (!lore1)
    return message.channel.send(
      new Discord.MessageEmbed()
      .setAuthor(message.author.tag, message.author.avatarURL)
      .setDescription(`**Argümanları Eksiksiz Doldur !** \`Örnek\`: .kadın @RK/id İsim Yaş`)
      .setColor("2F3136")
      );
  let role = message.guild.roles.cache.get(config.kadın);
  ...
  if (!nick)
    return message.channel.send(
      new Discord.MessageEmbed()
      .setAuthor(message.author.tag, message.author.avatarURL)
      .setDescription(`**Argümanları Eksiksiz Doldur !** \`Örnek\`: .kadın @RK/id İsim `)
      .setColor("2F3136")
    );
    const yas = args[2];
    if (!nick)
...
const Tagisim = `★ ${nick} | ${yas}`;
  ...
  setTimeout(function() {
    ...
    lore.roles.add(config.kadın1);
    lore.roles.add(config.kadın2);
    lore.roles.add(config.kadın3);
...
${lore.user} adlı kullanıcı sunucumuza __Kadın__ olarak kayıt edildi.
...
`);
 ...
 embed.description += `Geçmiş İsimleri Kontrol Etmek İçin .isimler @RK/id Yazıp Kontrol Edebilirsin \n\n Relly ❤️ Kyle. `
  ...
  ${lore.user} adlı kullanıcı başarılı bir şekilde __Kadın__ olarak kayıt edildi.
